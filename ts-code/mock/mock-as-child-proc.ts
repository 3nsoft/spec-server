...
const DEFAULT_DATA_FOLDER = join(__dirname, '/../../mock-data');
...
async function start(
	env: RequestEnvelope<MockConfAtStart>
): Promise<ServiceUrls> {
	...
	const noTokensFilePath = join(dataFolder, 'no-tokens-signup.json');
	const conf: Configurations = {
		...
		mailerId: {
			certs: join(dataFolder, 'mid-certs.json')
		}
	};
	...
	const noTokenCtx = makeMultiDomainSignupCtx(env.req.signupDomains);
	await writeFile(
