...
describe('Admin', () => {
	...
	describe('signup subsystem', () => {
		...
		function makeBadAddUserReq(goodId: string): any[] {
			...
			delete (req as any).storage;
			badJSONs.push(req);
			req = makeAddUserReq(goodId);
			delete req.storage.kdParams;
			badJSONs.push(req);
			req = makeAddUserReq(goodId);
			delete (req as any).mailerId;
			badJSONs.push(req);
			req = makeAddUserReq(goodId);
			delete (req.mailerId as any).defaultPKey;
			badJSONs.push(req);
			req = makeAddUserReq(goodId);
			delete (req.mailerId.defaultPKey.pkey as any).alg;
			badJSONs.push(req);
			req = makeAddUserReq(goodId);
			delete (req.mailerId.defaultPKey.pkey as any).k;
			badJSONs.push(req);
			req = makeAddUserReq(goodId);
			delete (req.mailerId.defaultPKey.pkey as any).kid;
			badJSONs.push(req);
			req = makeAddUserReq(goodId);
			delete (req.mailerId.defaultPKey.pkey as any).use;
			badJSONs.push(req);
			req = makeAddUserReq(goodId);
			delete req.mailerId.defaultPKey.kdParams;
			badJSONs.push(req);
			req = makeAddUserReq(goodId);
			delete (req.mailerId as any).otherPKeys;
			badJSONs.push(req);
			req = makeAddUserReq(goodId);
			req.mailerId.otherPKeys.shift();
			badJSONs.push(req);
			req = makeAddUserReq(goodId);
			delete (req.mailerId.otherPKeys[0] as any).alg;
			badJSONs.push(req);
			req = makeAddUserReq(goodId);
			delete (req.mailerId.otherPKeys[0] as any).k;
			badJSONs.push(req);
			req = makeAddUserReq(goodId);
			delete (req.mailerId.otherPKeys[0] as any).kid;
			badJSONs.push(req);
			req = makeAddUserReq(goodId);
			delete (req.mailerId.otherPKeys[0] as any).use;
			badJSONs.push(req);
			return badJSONs;
		}
