...
import { startSession, SpecDescribe, TestSetup, User, StorageComponent, listArchived, archiveObjVer } from '../test-utils';
...
export const specs: SpecDescribe = {
	description: `Request to archive object's versions`
};
...
specs.definition = (setup: () => TestSetup) => (() => {
	...
	beforeEachAsync(async () => {
		...
		reqOpts = {
			url: resolveUrl(
				...
				api.postAndDelReqUrlEnd(obj.objId, obj.version)),
			method: 'DELETE',
			sessionId
		};
	});
	...
	itAsync('fails when object does not exist', async () => {
		...
		opts.url = resolveUrl(
			user.storageOwnerUrl,
			api.postAndDelReqUrlEnd(unknownObj, 42));
