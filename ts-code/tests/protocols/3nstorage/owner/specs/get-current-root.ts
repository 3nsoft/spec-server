...
readSimpleObjVersion.definition = (setup: () => TestSetup) => (() => {
	...
	itAsync('returns header and all segments', async () => {
		expect(await storageServer.currentRootObjExists(user.id, obj.version, obj)).toBe(true, 'object must be in serve\'s store');
		const opts = copy(reqOpts);
...
accessDiffedObjVersion.definition = (setup: () => TestSetup) => (() => {
	...
	itAsync('returns header and all segments', async () => {
		expect(await storageServer.currentRootObjExists(user.id, diffVer.version, diffVer)).toBe(true, `object must be in serve's store`);
		const opts = copy(reqOpts);
