...
import { ErrorReply } from '../../lib-common/service-api/3nstorage/owner';
import { attachByteDrainToRequest } from '../../lib-server/middleware/body-parsers';

export function getObjIdFromParams(rootObj: boolean, req: Request): {
	objId: string|null; objIdParseErr?: string;
} {
	if (rootObj) {
		return { objId: null };
	} else {
