...
import { SC as recipSC, MsgRetrieval, BIN_TYPE } from '../../resources/recipients';
...
function extractQueryOptions(req: Request): undefined|{
	...
	if (isNaN(ofs) || (ofs < 0) || (header && (ofs > 0))) {
		return;
	}

	const limit = (query.limit ? parseInt(query.limit as any) : undefined);
	if (limit !== undefined) {
		if (isNaN(limit) || (limit < 0) || (!header && (limit === 0))) {
			return;
		}
	}
...
export function getMsgObj(
	getMsgObjFunc: MsgRetrieval['getObj']
): RequestHandler {
		...
		try{
			...
			if (reader.len > 0) {
				try {
					...
					console.error(errWithCause(
						err, `Wasn't able to complete sending segments due to error in piping`
					));
				} finally {
					res.end();
				}
