...
const NO_TOKENS_FILE = 'NO_TOKENS_FILE';
...
function makeConf(
	domain: string, rootFolder: string, midCertsFile: string,
	noTokenFile?: string
): Configurations {
	return {
		enabledServices: {
...
export function getConfsFromEnv(): Configurations {
	...
	const noTokenFile = process.env[NO_TOKENS_FILE];

	return makeConf(domain, dataDir, midCerts, noTokenFile);
}
