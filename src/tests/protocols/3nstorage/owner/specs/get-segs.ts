...
import { startSession, SpecDescribe, TestSetup, User, StorageComponent }
	from '../test-utils';
import { objSegs as api, HTTP_HEADER, ERR_SC }
...
let description = 'Request to get non-root object segments';
...
async function setStage(setup: () => TestSetup): Promise<void> {
	...
	reqOpts = {
		url: resolveUrl(user.storageOwnerUrl, api.getReqUrlEnd(obj.objId, obj.version)),
		method: 'GET',
		sessionId,
...
readSimpleObjVersion.definition = (setup: () => TestSetup) => (() => {
	...
	itAsync('fails when object does not exist', async () => {
		...
		let opts = copy(reqOpts);
		opts.url = resolveUrl(user.storageOwnerUrl, api.getReqUrlEnd(unknownObj, 1));
	...
	itAsync('will not accept bad parameters', async () => {
		let opts = copy(reqOpts);
		opts.url = resolveUrl(user.storageOwnerUrl, api.getReqUrlEnd(obj.objId, obj.version, { ofs: -34 }));
		let rep = await doBodylessRequest<Uint8Array>(opts);
		expect(rep.status).toBe(ERR_SC.malformed);
		opts.url = resolveUrl(user.storageOwnerUrl, api.getReqUrlEnd(obj.objId, obj.version, { len: -34 }));
		rep = await doBodylessRequest<Uint8Array>(opts);
		expect(rep.status).toBe(ERR_SC.malformed);
		opts.url = resolveUrl(user.storageOwnerUrl, api.getReqUrlEnd(obj.objId, obj.version, { ofs: 10, len: 0 }));
		rep = await doBodylessRequest<Uint8Array>(opts);
		expect(rep.status).toBe(ERR_SC.malformed);
	});
	...
	itAsync('returns chunk, when offset and length are given', async () => {
		...
		for (let offset=0; offset <= obj.segs.length; offset+=512) {
			...
			opts.url = resolveUrl(user.storageOwnerUrl, api.getReqUrlEnd(obj.objId, obj.version, { ofs: offset, len: chunkLen }));
			let rep = await doBodylessRequest<Uint8Array>(opts);
		...
		opts.url = resolveUrl(user.storageOwnerUrl, api.getReqUrlEnd(obj.objId, obj.version, { ofs: offset, len: requestedLen }));
		let rep = await doBodylessRequest<Uint8Array>(opts);
	...
	itAsync('reads segments from start, when offset is not given', async () => {
		...
		// with non-overflowing length
		let chunkLen = 512;
		opts.url = resolveUrl(user.storageOwnerUrl, api.getReqUrlEnd(obj.objId, obj.version, { len: chunkLen }));
		let rep = await doBodylessRequest<Uint8Array>(opts);
		checkReplyExpectation(rep, obj, obj.segs.subarray(0, chunkLen));
		// with overflowing length
		opts.url = resolveUrl(user.storageOwnerUrl, api.getReqUrlEnd(obj.objId, obj.version, { len: obj.segs.length+300 }));
		rep = await doBodylessRequest<Uint8Array>(opts);
		checkReplyExpectation(rep, obj, obj.segs);
	});
	...
	itAsync('reads segments to the end, when length is not given', async () => {
		let opts = copy(reqOpts);
		opts.url = resolveUrl(user.storageOwnerUrl, api.getReqUrlEnd(obj.objId, obj.version, { ofs: 1234 }));
		let rep = await doBodylessRequest<Uint8Array>(opts);
	...
	itAsync('no-bytes read', async () => {
		let opts = copy(reqOpts);
		opts.url = resolveUrl(user.storageOwnerUrl, api.getReqUrlEnd(obj.objId, obj.version, { ofs: obj.segs.length, len: 500 }));
		let rep = await doBodylessRequest<Uint8Array>(opts);
...
accessDiffedObjVersion.definition = (setup: () => TestSetup) => (() => {
	...
	beforeAllAsync(async () => {
		...
		reqOpts = {
			url: resolveUrl(user.storageOwnerUrl, api.getReqUrlEnd(diffVer.objId, diffVer.version)),
			method: 'GET',
			sessionId,
			responseType: 'arraybuffer',
			responseHeaders: [ HTTP_HEADER.objSegmentsLength, HTTP_HEADER.objVersion ]
	...
	itAsync('returns chunk, when offset and length are given', async () => {
		...
		let requestedLen = 300
		opts.url = resolveUrl(user.storageOwnerUrl, api.getReqUrlEnd(diffVer.objId, diffVer.version, { ofs: offset, len: requestedLen }));
		let rep = await doBodylessRequest<Uint8Array>(opts);
