...
import { startSession, SpecDescribe, TestSetup, User, StorageComponent }
	from '../test-utils';
import { startTransaction as api, ERR_SC }
	from '../../../../../lib-common/service-api/3nstorage/owner';
import { beforeAllAsync, beforeEachAsync, itAsync, xitAsync }
...
let description = 'Request to start non-root object transaction';
...
specsForNewObject.definition = (setup: () => TestSetup) => (() => {
	...
	itAsync('will not start transaction without new-object flag', async () => {
		expect(await storageServer.transactionExists(user.id, obj.objId)).toBeFalsy('should be no transaction at the begining.');
		expect(await storageServer.objExists(user.id, obj.objId)).toBeFalsy('object is not present at the begining.');
		let req: api.Request = {
		...
		expect (await storageServer.transactionExists(user.id, obj.objId)).toBeFalsy('should be no transaction, as starting it should have failed.');
	});
	...
	itAsync('starts transaction', async () => {
		...
		expect (await storageServer.transactionExists(user.id, obj.objId, transactionId)).toBeTruthy('transaction has been created');
	});
	...
	itAsync('will not start concurrent transaction', async () => {
		...
		expect (await storageServer.transactionExists(user.id, obj.objId, transactionId)).toBeTruthy('transaction stays untouched');
	});
...
specsExistingObject.definition = (setup: () => TestSetup) => (() => {
	...
	itAsync('starts transaction for next version', async () => {
		expect(await storageServer.objExists(user.id, obj.objId, obj.version)).toBeTruthy('object version already exists');
		let req: api.Request = {
			sizes: { header: obj.header.length, segments: obj.segs.length },
			version: obj.version + 1
		};
		let rep = await doJsonRequest<api.Reply>(reqOpts, req);
		expect(rep.status).toBe(api.SC.ok, 'normal start of a transaction');
		expect (await storageServer.transactionExists(user.id, obj.objId, rep.data.transactionId)).toBeTruthy('transaction has been created');
	});
...
let specsForObjIdRestriction: SpecDescribe = { description };
specs.push(specsForObjIdRestriction);
specsForObjIdRestriction.definition = (setup: () => TestSetup) => (() => {
	...
	itAsync('does not allow new object with existing id',
			async () => {
		expect(await storageServer.objExists(user.id, obj.objId)).toBeTruthy(`object has been added with an id ${obj.objId}`);
		...
		let rep = await doJsonRequest<api.Reply>(reqOpts, req);
		expect(rep.status).toBe(api.SC.objAlreadyExists, 'status for an already existing object with a given id');
	});
	
	itAsync('does not allow objects with ids, same up to a letter case',
			async () => {
		expect(await storageServer.objExists(user.id, obj.objId)).toBeTruthy(`object has been added with an id ${obj.objId}`);
		...
		let idWithDifferentCase = obj.objId.toUpperCase();
		reqOpts.url = resolveUrl(user.storageOwnerUrl,
			api.getReqUrlEnd(idWithDifferentCase));
		let rep = await doJsonRequest<api.Reply>(reqOpts, req);
		expect(rep.status).toBe(api.SC.objAlreadyExists, 'status for an already existing object with a given id, since ids only differ in letter case(s)');
	});
