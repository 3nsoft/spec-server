...
import { startSession, SpecDescribe, TestSetup, User, StorageComponent }
	from '../test-utils';
import { rootHeader as api, ERR_SC }
...
async function setupWithNonDiffTransaction(
	...
	transactionId = await startTransaction(user.storageOwnerUrl,
		sessionId, obj, true);
	reqOpts = {
		url: resolveUrl(user.storageOwnerUrl, api.putReqUrlEnd(transactionId)),
		method: 'PUT',
		sessionId
	};
}
...
fuzzingSpec.definition = (setup: () => TestSetup) => (() => {
	...
	itAsync('fails for incorrect transaction', async () => {
		...
		opts.url = resolveUrl(user.storageOwnerUrl, api.putReqUrlEnd('unknown-transaction'));
...
let specsForNonDiffTransaction: SpecDescribe = { description };
