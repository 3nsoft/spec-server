...
import { ICompleteTransaction, ICancelTransaction, SC as storeSC }
	from '../../resources/users';
import { finalizeTransaction as api, ERR_SC } from '../../../lib-common/service-api/3nstorage/owner';
import { Request } from '../../../lib-server/routes/sessions/start';

export function closeTransaction(root: boolean, normalClosing: boolean,
		closeTransFunc: ICompleteTransaction|ICancelTransaction): RequestHandler {
	...
	return async function(req: Request, res: Response, next: NextFunction) {
		...
		let transactionId: string = req.params.transactionId;
		
		try {
			if (normalClosing) {
				await (<ICompleteTransaction> closeTransFunc)(
					userId, objId, transactionId);
			} else {
				await (<ICancelTransaction> closeTransFunc)(userId, objId);
			}
