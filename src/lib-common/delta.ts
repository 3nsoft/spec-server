...
export class Delta {
	...
	complete(): boolean {
		...
		let ifLastSkip = this.ifSize - this.byteCount.ifP;
		let rfLastSkip = this.rfSize - this.byteCount.rfP;
	...
	insertBytes(len: number): ChangeOp {
		...
		let prevCh = ((this.changes.length > 0) ?
	...
	packDeltaFileHeader(): Uint8Array {
		...
		let jsonBytes = utf8.pack(JSON.stringify(json))
		let header = new Uint8Array(jsonBytes.length);
	...
	static makeFromJSON(json) {
		...
		let d =  new Delta(json.id, json.obj,
		...
		json.changes.forEach((ch, i) => {
			...
			if (ch.op === Delta.INSERT) {
				let insCh = d.insertBytes(ch.len);
				if (ch.bP !== 0) {
	...
	makeFlippedDelta(id: string) {
		...
		let flippedDelta =  new Delta(id, this.objId,
	...
	static mergeDeltas(id: string, d12: Delta, d23: Delta) {
		...
		function merge12Insert(ins12, byteOffset?) {
			let insByteOffset = (('number' === typeof byteOffset) ?
					byteOffset : 0);
			let ins12end = ins12.rfP + ins12.len;
			let x, ch13, overlap;
		...
		function merge23Delete(del23, byteOffset?) {
			let delByteOffset = (('number' === typeof byteOffset) ? byteOffset : 0)
			let del23end = del23.ifP + del23.len
			let x, overlap;
		...
		for (let i=0; i<orderedChanges.length; i+=1) {
			x = orderedChanges[i];
...
function orderChangesAlongP2(d12, d23) {
	...
	if (i12 > 0) {
		for (let i=i12; i<d12.changes.length; i+=1) {
			orderedChanges.push({ first: true, ch: d12.changes[i] });
		}
	}
	if (i23 > 0) {
		for (let i=i23; i<d23.changes.length; i+=1) {
